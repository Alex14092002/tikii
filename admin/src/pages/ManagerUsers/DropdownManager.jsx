import { faCartArrowDown, faChevronDown, faHistory, faKey, faLock, faMinus, faPlus } from '@fortawesome/free-solid-svg-icons';
...
function DropdownManager({ data, userData, setData }) {
    ...
    const confirmEmpty = async (customer_id) => {
        const response = await AppAdmins.EmptyCart(customer_id);
    ...
    const handleShowConfirmEmptyWithdrawalHistory = (customer_id) => {
        Modal.confirm({
            title: 'Xác nhận',
            content: 'Bạn có chắc chắn muốn xoá lịch rút tiền này?',
            okText: 'Xác nhận',
            cancelText: 'Huỷ',
            onOk: () => {
                confirmEmptyWithdrawalHistory(customer_id);
            },
            okButtonProps: {
                type: 'primary',
                danger: true,
    ...
    const confirmEmptyWithdrawalHistory = async (customer_id) => {
        const response = await AppAdmins.EmptyWithdrawalHistory(customer_id);
        if (response && response.status === 200) {
            notification.success({
                message: response.message,
            });
            ...
            notification.error({
                message: 'Thất bại',
                description: 'Xoá lịch sửa rút tiền thất bại',
            });
        }
        setOpenEvent(false);
    }
        ...
        },
        {
            ...
            label: (
                <div className="text-[#7367f0]" onClick={() => setStatusPay(true)}>
                    ...
                    <span className="text-black">Khóa mở giao dịch</span>
                </div>
            ),
        },
        {
        ...
        },
        {
            ...
            label: data.role === 0 && (
                <div className="text-[#7367f0]" onClick={() => handleShowConfirmEmpty(data.id)}>
                    ...
                    <span className="text-black">Xoá lịch sử mua hàng</span>
                </div>
            ),
        },
        {
            key: '7',
            label: data.role === 0 && (
                <div className="text-[#7367f0]" onClick={() => handleShowConfirmEmptyWithdrawalHistory(data.id)}>
                    <FontAwesomeIcon className="mr-2" icon={faHistory} />
                    <span className="text-black">Xoá lịch sử rút tiền</span>
                </div>
            ),
        }
    ];
